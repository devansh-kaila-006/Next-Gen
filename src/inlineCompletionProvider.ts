...
export class AgenticInlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  async provideInlineCompletionItems(
    ...
    const model = genAI.getGenerativeModel({ model: "gemini-3.5-flash", systemInstruction: "You are an inline code autocomplete agent. Only output the code that should be inserted at the cursor. Do NOT include markdown code blocks (```) or any conversational text. Keep the suggestion brief (1 to 5 lines maximum)." });
